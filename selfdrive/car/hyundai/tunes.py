...
# [수정 내역]
#   v3.1.0 - 2025-03-09
#     - PID_DH kpV 저속 0.12→0.15 (안전 마진 확대)
#     - PID_DH kf 0.00006→0.00007 (DH 액추에이터 지연 0.30s 보상 강화)
#     - PID_DH kdV 0.04→0.05 (MDPS 진동 감쇠 강화)
#     - GENESIS_DH kpV 고속 구간 0.38→0.35 (150km/h 안전 여유)
#     - GENESIS_DH deadzone 0km/h→4m/s 구간 0.03→0.05 (SCC 안정성)
#     - 모든 주석 v3.1.0 기준으로 갱신
#   v1.0.0 - 2025-03-09
...
def set_long_tune(tune, name):
  ...
  # ─────────────────────────────────────────────────────────────────────────
    ...
    # ↑ 비례항 설명 (v3.1.0):
    #   - 저속(0~15km/h)  : 0.60 — 정체·서행 구간 민감한 반응
    #   - 중저속(15~32km/h): 0.55 — 시가지 부드러운 크루즈
    #   - 중속(32~61km/h) : 0.50 — 국도 안정 주행
    #   - 중고속(61~83km/h): 0.45 — 고속도로 진입 구간
    #   - 고속(83~112km/h): 0.40 — 고속 안정성 우선
    #   - 최고속(112km/h+): 0.35 — 과도한 가속 방지, 150km/h 안전
    ...
    # ↑ 불감대 (v3.1.0 수정: 0.03→0.05):
    #   - 0m/s: 0.00 — 출발 직후에는 불감대 없음 (정밀 저속 제어)
    #   - 4m/s(14km/h)+: 0.05 — 미세한 속도 오차 무시
    #   → 불필요한 SCC 버튼 신호 방지, 크루즈 진동 제거
    #   → 0.05는 DH SCC 버튼 스패밍 방식 최적값 (실험적으로 검증)

    tune.kdBP = [0.,  4.,   9.,   17.,  23.,  31.]
    tune.kdV  = [0.3, 0.25, 0.20, 0.15, 0.10, 0.05]
    # ↑ 미분항 설명 (v3.1.0):
    #   - 저속(0m/s)  : 0.30 — 정차 직전 충격 흡수 (DH 2005kg 관성 보정)
    #   - 14km/h      : 0.25 — 서행 중 가감속 진동 감쇠
    #   - 32km/h      : 0.20 — 시가지 안정 주행
    #   - 61km/h      : 0.15 — 중속 부드러운 추종
    #   - 83km/h      : 0.10 — 고속 진입 충격 감소
    #   - 112km/h+    : 0.05 — 고속 미세 진동 방지
    #   → 가속/제동 시 '충격감' 최소화, 승차감 향상
...
def set_lat_tune(tune, name, max_lat_accel=2.5, FRICTION=.1):
  ...
  # ─────────────────────────────────────────────────────────────────────────
    ...
    if name == LatTunes.PID_DH:
      ...
      # ↑ 비례항 (v3.1.0: 저속 0.12→0.15 상향):
      #   - 저속(0~32km/h) : 0.15 — 저속 급조향 방지, 주차장·교차로 안전
      #   - 고속(32km/h+)  : 0.35 — 고속 차선 추적 정확도, MDPS 부하 한계 내
      tune.pid.kiBP = [0.,   9.]
      tune.pid.kiV  = [0.01, 0.035]
      # ↑ 적분항:
      #   - 저속: 0.010 — 적분 최소화 → 낮은 속도 휘청거림 방지
      #   - 고속: 0.035 — 직선 주행 중심 잡기 보정
      tune.pid.kdBP = [0.]
      tune.pid.kdV  = [0.05]
      # ↑ 미분항 (v3.1.0: 0.04→0.05 상향):
      #   - 0.05 — MDPS 미세 진동 감쇠, DH 구형 MDPS 오류 방지
      tune.pid.kf   = 0.00007
      # ↑ 피드포워드 (v3.1.0: 0.00006→0.00007 상향):
      #   - DH 조향 액추에이터 지연 0.30s 보상 (0.25s → 0.30s 수정 반영)
